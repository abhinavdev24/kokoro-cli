...
pub struct KokoroModelParams {
    ...
    /// Path to a custom voice file.
    ///
    /// - **`.npy`** — a single voice; the voice name is derived from the file stem
    ///   (e.g. `af_heart.npy` → use `"af_heart"` in [`KokoroInferenceParams::voice`]).
    /// - **`.npz` / `.bin` / any zip** — a multi-voice archive in the standard Kokoro
    ///   format; all contained voices are loaded.
    ///
    /// When `None` (the default) the engine looks for `voices-v1.0.bin` inside the
    /// model directory.
...
impl KokoroEngine {
    ...
    /// Return the name of the first loaded voice (requires model to be loaded).
    ///
    /// Useful after loading a single-voice file to discover the derived voice name.
    pub fn first_voice_name(&self) -> Option<&str> {
        self.model.as_ref().and_then(|m| m.first_voice_name())
    }
}
