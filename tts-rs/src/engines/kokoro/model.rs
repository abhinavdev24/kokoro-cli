...
impl KokoroModel {
    ...
    pub fn load(
        ...
        // Load voices: explicit path overrides the default voices-v1.0.bin lookup.
        let voice_store = if let Some(path) = voice_file {
            if !path.exists() {
                return Err(KokoroError::Io(std::io::Error::new(
                    std::io::ErrorKind::NotFound,
                    format!("Voice file not found at {}", path.display()),
                )));
            }
            load_voice_file(path)?
        } else {
            let voices_path = find_voice_file(model_dir)?;
            load_voice_file(&voices_path)?
        };
    ...
    /// Return the name of the first loaded voice, if any.
    ///
    /// Handy when a single voice file was loaded and the caller needs the
    /// derived name (file stem) without inspecting the path themselves.
    pub fn first_voice_name(&self) -> Option<&str> {
        self.voice_store.first_voice_name()
    }
}

/// Find a voice file in the model directory.
///
/// Prefers `voices-v1.0.bin`, then falls back to the first `.bin`, `.npz`, or `.npy`
/// file found, so that a single downloaded voice file (e.g. `voice.bin`) works without
/// any extra flags.
fn find_voice_file(model_dir: &Path) -> Result<std::path::PathBuf, KokoroError> {
    let preferred = model_dir.join("voices-v1.0.bin");
    if preferred.exists() {
        return Ok(preferred);
    }
    ...
    for entry in std::fs::read_dir(model_dir)? {
        ...
        if path.is_file() {
            match path.extension().and_then(|e| e.to_str()) {
                Some("bin") | Some("npz") | Some("npy") => {
                    log::info!("Using voice file: {}", path.display());
                    return Ok(path);
                }
                _ => {}
            }
    ...
    Err(KokoroError::Io(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        format!(
            "No voice file found in {}. Expected voices-v1.0.bin or any .bin/.npz/.npy file.",
            model_dir.display()
        ),
    )))
}

/// Detect the format of a voice file and load it into a [`VoiceStore`].
///
/// Detection order:
/// 1. `.npy` extension → numpy single-voice file
/// 2. ZIP magic bytes (`PK\x03\x04`) → multi-voice `.npz` / `.bin` archive
/// 3. Anything else → raw little-endian f32 single-voice binary (HuggingFace format)
fn load_voice_file(path: &Path) -> Result<VoiceStore, KokoroError> {
    if path.extension().and_then(|e| e.to_str()) == Some("npy") {
        return VoiceStore::load_single_npy(path);
    }

    // Peek at the first 4 bytes to check for a ZIP signature.
    let mut header = [0u8; 4];
    {
        use std::io::{Read as _, Seek as _, SeekFrom};
        let mut f = std::fs::File::open(path)?;
        let n = f.read(&mut header)?;
        // Rewind is not strictly needed (we open a fresh handle below), but be explicit.
    ...
    // ZIP files start with the local-file-header signature 50 4B 03 04.
    if &header == b"PK\x03\x04" {
        VoiceStore::load(path)
    } else {
        // Assume raw f32 (Kokoro ONNX HuggingFace voice format).
        VoiceStore::load_raw_f32(path)
    }
}
...
fn init_session(
    ...
    if write_cache {
        ...
        builder = builder.with_optimized_model_path(cache).unwrap();
    }
